...
Expr Parser::parsePrimaryExpr(){
    ...
    if (at().token == other){
        return VarExpr{eat().value};
    } else if (at().token == number){
        return NumExpr{std::stoi(eat().value)};
    } else if (at().token == openParen){
        eat();
        Expr expr = parseExpr();
        expect(closeParen, "Expected close parenthesis");
        return expr;
    } else {
        logError("Unknown token", 0);
        return {};
    }
}
